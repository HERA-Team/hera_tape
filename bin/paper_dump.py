...
class Dump:
    ...
    def archive_to_tape(self):
        ...
        while self.tape_used_size + self.batch_size_mb < self.tape_size:
            ...
            if archive_list:
                ...
                self.debug.print('list - %s' % archive_list)
                ...
                self.debug.print("q:%s l:%s t:%s" % (self.tape_used_size, archive_size, self.tape_size))
        ...
        if self.tape_used_size > 0:
            self.debug.print('sending queued files to tar - %s, %s' % (len(self.files.tape_list), self.files.tape_list))
    ...
    def tar_archive_single(self, catalog_file):
        ...
        for label_id in tape_label_ids:
            ...
            for tape_index in range(self.tape_index):
                self.debug.print('sending tar to single drive', str(tape_index), debug_level=225)
                try:
    ...
    def tar_archive(self, catalog_file):
        ...
        self.debug.print('writing tape_indexes - %s' % self.files.tape_list)
        self.paperdb.write_tape_index(self.files.tape_list, ','.join(tape_label_ids))
    ...
    def batch_files(self, queue=False, regex=False, pid=False, claim=True):
        ...
        while self.tape_used_size + self.batch_size_mb < self.tape_size:
            ...
            if file_list:
                ...
                self.debug.print("queue list: %s, len(catalog_list): %s" % (str(self.tape_index), len(self.files.catalog_list)))
    ...
    def manual_to_tape(self, tape_index, cumulative_list):
        ...
        self.debug.print("manual vars - qp:%s, cn:%s" % (tape_index, cumulative_list))
        self.tar_archive_single(self.files.catalog_name)
